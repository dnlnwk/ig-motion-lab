...
- Built with `motion` (`motion/react`): the pill ↔ circle width morph is an animated `width` value using two different spring transitions — a well-damped, non-bouncy spring when shrinking into the circle, and a slightly bouncier one when expanding back into the pill — and icon/label swaps use `AnimatePresence` for crossfades.
...
- The checkmark is an inline SVG `path` animated via motion's `pathLength` (0 → 1) for a draw-in effect, layered inside a container that also scales/fades in with a spring.
